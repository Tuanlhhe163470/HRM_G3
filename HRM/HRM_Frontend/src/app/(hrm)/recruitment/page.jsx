...
export default function RecruitmentDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const needScheduleCount = candidates.filter(
        (c) =>
          c.status === "Interview" &&
          !interviews.some((i) => i.candidateID === c.candidateID),
      ).length;

      const needOfferCount = candidates.filter(
        (c) => c.status === "Passed",
      ).length;
  ...
  return (
    <div className={styles.dashboardContainer}>
      <Card variant="borderless" className={styles.mainCard}>
        <h1 className="text-[#154398] uppercase font-black">
          Tổng hợp thông tin tuyển dụng
        </h1>
        <Text type="secondary" className="block mb-6 italic">
          Dữ liệu tính đến: {dayjs().format("HH:mm:ss DD/MM/YYYY")}
        </Text>

        <Row gutter={[16, 16]}>
          <Col span={6}>
            <Card className={styles.statCard} hoverable>
              <Link href="/recruitment/job-postings/list" className="block">
                <Statistic
                  title="Tin đang mở"
          ...
          <Col span={6}>
            <Card className={styles.statCard} hoverable>
              <Link href="/recruitment/interview/hr-schedule" className="block">
                {" "}
                <Statistic
          ...
          <Col span={6}>
            <Card className={styles.statCard} hoverable>
              <Link href="/recruitment/offer/create" className="block">
                <Statistic
        ...
        <Row gutter={[16, 16]} style={{ marginTop: "24px" }}>
          <Col span={16}>
            <Card
              title={
                <Text strong className="text-[#154398]">
                  <RiseOutlined /> Hồ sơ mới chưa xử lý
                </Text>
              }
            ...
            >
              <div className={styles.pipelineItem}>
                <span className={styles.pipelineLabel}>
                  <ClockCircleOutlined /> Đang lọc hồ sơ
                </span>
              ...
              <div className={styles.pipelineItem}>
                <span className={styles.pipelineLabel}>
                  <HourglassOutlined /> Đang chờ duyệt
                </span>
                <span
                  className={styles.pipelineValue}
                  style={{ color: "#faad14" }}
                >
              ...
              <div className={styles.pipelineItem}>
                <span className={styles.pipelineLabel}>
                  <UsergroupAddOutlined /> Phỏng vấn
                </span>
                <span className={styles.pipelineValue}>
                  {pipeline.interview}
                </span>
              ...
              <div className={styles.pipelineItem}>
                <span className={styles.pipelineLabel}>
                  <CloseCircleOutlined /> Hồ sơ loại
                </span>
                <span
                  className={styles.pipelineValue}
                  style={{ color: "#ff4d4f" }}
