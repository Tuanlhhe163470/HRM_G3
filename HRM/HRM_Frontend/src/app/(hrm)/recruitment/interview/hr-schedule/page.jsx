...
import {
  Typography, Tag, Avatar, Card, Button, Modal,
  Form, DatePicker, Select, Input, App, Empty, Spin,
} from "antd";
import {
  UserOutlined, CalendarOutlined, ClockCircleOutlined,
  EnvironmentOutlined, PlusOutlined, PhoneOutlined,
  CheckCircleOutlined, SolutionOutlined, MailOutlined
...
export default function InterviewSchedulePage() {
    ...
    const handleNavigation = async () => {
      ...
      try {
        ...
        const latestInv = allInterviews
          ...
          .sort((a, b) => new Date(b.interviewDate) - new Date(a.interviewDate))[0];
        ...
        notification.error({ message: "Lỗi", description: "Không tải được thông tin ứng viên." });
      } finally {
  ...
  const checkIsScheduled = (id) => interviews.some(inv => inv.candidateID === parseInt(id));
  ...
  const calendarDays = (() => {
    ...
    const days = [];
    for (let i = 0; i < firstDayIndex; i++) days.push({ type: "empty", key: `empty-${i}`, date: null });
    for (let i = 1; i <= daysInMonth; i++) {
      const date = new Date(year, month, i);
      const hasEvent = interviews.some(inv => dayjs(inv.interviewDate).isSame(dayjs(date), "day"));
    ...
    <div className="mx-auto max-w-[1440px] p-6 lg:p-10 flex flex-col gap-8 text-slate-900 text-left">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="grid grid-cols-7 bg-slate-100 gap-[1px]">
            {calendarDays.map((item) => {
              ...
              const isSelected = item.date && dayjs(item.date).isSame(dayjs(selectedDay.date), 'day');
              return (
                <div
                  key={item.key}
                ...
                >
                  ...
                  {item.hasEvent && <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-purple-500 rounded-full shadow-sm"></div>}
                </div>
              );
            })}
          </div>
        ...
        <div className="w-full lg:w-[420px] flex flex-col gap-6">
          <Card className="rounded-2xl shadow-sm border-none bg-white overflow-hidden" title={<span className="font-bold text-[#154398]">Thông tin ứng viên</span>}>
            {loadingInfo ? <div className="py-10 text-center"><Spin /></div> : candidateInfo ? (
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-4">
                  <Avatar size={64} icon={<UserOutlined />} className="bg-blue-100 text-blue-600 border-2 border-white shadow-sm flex-shrink-0" />
                  <div className="flex-1 overflow-hidden">
                    <div className="flex items-center gap-2">
                        <Text strong className="text-xl block leading-tight truncate">{candidateInfo.fullName}</Text>
                        {checkIsScheduled(candidateInfo.candidateID) && <CheckCircleOutlined className="text-green-500 text-lg" />}
                    </div>
              ...
              <div className="max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {waitingCandidates.length > 0 ? waitingCandidates.map((item) => {
                    const isScheduled = checkIsScheduled(item.candidateID);
                    return (
                        <div key={item.candidateID} className="p-3 hover:bg-blue-50 cursor-pointer rounded-xl mb-2 flex items-center gap-3 bg-white shadow-sm transition-all" onClick={() => router.push(`?candidateId=${item.candidateID}`)}>
                            <div className="relative">
                                <Avatar icon={<UserOutlined />} className="bg-blue-100 text-blue-600" />
                                {isScheduled && <CheckCircleOutlined className="absolute -top-1 -right-1 text-green-500 bg-white rounded-full text-[12px]" />}
                            </div>
                            <div className="flex flex-col overflow-hidden flex-1">
                                <div className="flex justify-between items-start">
                                    <Text strong className="text-sm truncate leading-tight">{item.fullName}</Text>
                ...
                }) : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="Không có ứng viên" />}
              </div>
            )}
          </Card>

          <Card className="rounded-2xl shadow-sm border-none bg-white flex-1" title={<span className="font-bold text-slate-700">Sự kiện ngày {selectedDay?.day}</span>}>
            <div className="space-y-4">
              {dailyInterviews.length > 0 ? dailyInterviews.map((inv, idx) => (
      ...
      <Modal title={<Title level={4} className="m-0 text-[#154398]">LỊCH HẸN PHỎNG VẤN</Title>} open={isModalOpen} onCancel={() => setIsModalOpen(false)} footer={null} centered>
        <Form form={form} layout="vertical" onFinish={handleSchedule} className="mt-4">
          <Form.Item label="Thời gian" name="interviewDate" rules={[{ required: true, message: 'Chọn thời gian!' }]}>
            <DatePicker showTime className="w-full h-11 rounded-xl" format="DD/MM/YYYY HH:mm" disabledDate={disabledDate} />
          </Form.Item>
          <Form.Item label="Hình thức" name="interviewType" initialValue="Online">
            <Select className="h-11">
              <Select.Option value="Online">Online (Google Meet/Zoom)</Select.Option>
              <Select.Option value="Offline">Offline (Tại văn phòng)</Select.Option>
            </Select>
          </Form.Item>
