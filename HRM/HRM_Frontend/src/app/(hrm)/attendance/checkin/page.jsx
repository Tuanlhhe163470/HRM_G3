...
export default function EmployeeDashboard() {
  ...
  const workdaysPercent = summary ? ((summary.actualWorkDays || 0) / 22) * 100 : 0;
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-black/5 dark:bg-slate-800 dark:ring-white/10 hover:shadow-md transition-shadow">
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-medium text-slate-900 dark:text-white">Ngày công thực tế</h3>
            </div>
            <span className="text-xs font-medium text-slate-500">Tháng này</span>
          </div>
          <div className="flex items-end justify-between">
            <span className="text-3xl font-bold text-slate-900 dark:text-white">
              {summary?.actualWorkDays || 0}<span className="text-xl font-medium text-slate-500">/22</span>
            </span>
          ...
          <div className="mt-4 h-2 w-full rounded-full bg-gray-100 dark:bg-gray-700 overflow-hidden">
            <div className="h-2 rounded-full bg-blue-600 transition-all duration-500" style={{ width: `${workdaysPercent}%` }}></div>
          </div>
        </div>
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-black/5 dark:bg-slate-800 dark:ring-white/10 hover:shadow-md transition-shadow">
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-medium text-slate-900 dark:text-white">Tổng phút đi muộn</h3>
            </div>
            <span className="text-xs font-medium text-slate-500">Tháng này</span>
          </div>
          <div className="flex items-end justify-between">
            <span className="text-3xl font-bold text-slate-900 dark:text-white">
              {summary?.totalLateMinutes || 0} <span className="text-lg font-normal text-slate-500">phút</span>
            </span>
            {summary?.totalLateMinutes > 0 ? (
               <span className="rounded-full bg-orange-100 px-2 py-0.5 text-xs font-medium text-orange-700">Cảnh báo</span>
            ) : (
               <span className="rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">Hoàn hảo</span>
            )}
          </div>
          <p className="mt-4 text-sm text-slate-500">
             {summary?.totalLateMinutes > 0 ? `Bạn đã đi muộn ${summary?.lateCount || 0} lần.` : 'Tuyệt vời! Bạn luôn đi làm đúng giờ.'}
          </p>
        </div>

        {/* Card 3: Số ngày phép (Phần này vẫn Hardcode vì chưa có DB Phép) */}
        <div className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-black/5 dark:bg-slate-800 dark:ring-white/10 hover:shadow-md transition-shadow">
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col rounded-xl bg-white p-6 shadow-sm ring-1 ring-black/5 dark:bg-slate-800 dark:ring-white/10">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
            ...
            <div className="min-w-[160px] flex-1 rounded-lg border border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50 hover:bg-purple-50 transition-colors cursor-pointer">
              <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-sm dark:bg-gray-700">
                <span className="material-symbols-outlined text-purple-500">celebration</span>
              </div>
